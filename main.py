...
app = FastAPI(title="CEPLAN Mapa de Oportunidades")
...
@app.get("/")
def root():
    return {"status": "ok", "service": "CEPLAN Mapa de Oportunidades"}

@app.post("/generateOportunidades")
async def generate_oportunidades(request: Request):
    try:
        body = await request.json()
    except Exception:
        raise HTTPException(status_code=400, detail="Body no es JSON valido")

    titulo = body.get("titulo", "Mapa de Oportunidades Globales y Nacionales")
    periodo = body.get("periodo", "2026-2036")
    oportunidades_json = body.get("oportunidades_json")

    if not oportunidades_json:
        raise HTTPException(status_code=400, detail="Falta oportunidades_json")

    if isinstance(oportunidades_json, str):
        try:
            oportunidades_json = json.loads(oportunidades_json)
        except Exception:
            raise HTTPException(status_code=400, detail="oportunidades_json no es JSON valido")
...
def view_oportunidades(session_id: str):
    cleanup_sessions()
    if session_id not in sessions:
